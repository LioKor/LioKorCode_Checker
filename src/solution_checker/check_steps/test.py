...
from src.solution_checker.models import CheckStatus, TestResult, TestsResult
...
def run_test(
    ...
    if result is None:
        ...
        test_thread.join()
        return TestResult(status=CheckStatus.EXECUTION_TIMEOUT, time=test_time, message="")

    exit_code, stdout = result
    if exit_code != 0:
        return TestResult(status=CheckStatus.RUNTIME_ERROR, time=test_time, message=stdout)
